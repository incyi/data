...
def check_files(files: List[str], reporters=None) -> List[str]:
    ...
    for file_path in files:
        errors.extend(check_file(file_path, reporters=reporters))
...
def check_file(file_path: str, reporters=None) -> List[str]:
    ...
    if not os.path.exists(file_path):
        ...
        report_error(file_path, "-", msg, errors, reporters=reporters)
        return errors
    ...
    for url in df["url"].dropna().astype(str):
        errors.extend(check_url(url, file_path, reporters=reporters))
...
def check_url(url: str, file_path: str, reporters=None) -> List[str]:
    ...
    try:
        ...
        if response.status_code >= 400:
            ...
            report_error(file_path, url, msg, errors, reporters=reporters)
        else:
            report_error(file_path, url, "OK", errors, reporters=reporters, ok=True)
    except RequestException as e:
        msg = f"{url} failed: {e}"
        report_error(file_path, url, msg, errors, reporters=reporters)

    # 2️⃣ Check SSL/TLS certificate if URL uses HTTPS
    parsed = urlparse(url)
...
def report_error(file_path: str, url: str, msg: str, errors: List[str], *, reporters=None, ok: bool = False):
